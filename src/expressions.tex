...
\enternote 
第~\ref{expr}~章定义表达式的语法、求值顺序和含义。\footnote{操作符的优先级没有直接指定，它隐含在语法中。}
表达式就是指明了一次计算的操作符和操作数序列。表达式可能产生值，还可能产生边界作用。
\exitnote 

\pnum
\indextext{operator!overloaded}%
\enternote 
操作符允许重载，是说当操作符应用在类类型~（第~\ref{class}~章）或枚举类型（\ref{dcl.enum}）的表达式上时给予其含义。重载操作符的使用都会转化为~\ref{over.oper}~节描述的函数调用。重载操作符遵循第~\ref{expr}~章的所有语法规则，但是其所需的操作数类型、值类别以及求值顺序都遵循的是函数调用的规则。（内置）操作符之间的关系，例如
\tcode{++a} 等价于 \tcode{a+=1}，在其重载版本（\ref{over.oper}）上不一定成立。
\exitnote 

\pnum
第~\ref{expr}~章定义了操作符的非重载版本的边界作用（更确切地说，定义了操作符用在使自身为非重载版本的类型上的边界作用）。操作符重载不能影响 \defnx{内置操作符}{operators!built-in}的规则，也就是说不能影响操作符用在本标准定义的这些类型上的规则。However, these built-in operators participate in overload
...
for compatibility with ISO \CppXIV. \exitnote
