...
\pnum
\enternote 本条款提出\Cpp 语言的基本概念。
在本条款中，解释了\term{对象}和\term{名称}之间的差异以及
它们如何关系到表达式的值类别；
介绍了\term{声明}和\term{定义}的概念，
并提出\Cpp 关于\term{类型}、\term{作用域}、\term{linkage}以及\term{存储} \term{周期}方面的概念。
本条款还讨论了程序启动和终止的机制。
在最后，本条款提出了语言的\term{基本}类型，
并列出由\term{基本}类型构造\term{复合}类型的途径。

\pnum
\enternote
本条款只涉及贯穿于整个语言的概念，
独立于语言某一部分的概念在相关条款中给出。
\exitnote
...
\indextext{entity}%
\defn{实体}是值、对象、引用、函数、枚举量、类型、
类成员、位域、模板、模板特化、命名空间或参包中的一种。

\pnum
\defn{名称}是对
表示实体或\grammarterm{标号}~(\ref{stmt.goto}, \ref{stmt.label})的
\grammarterm{标识符}~(\ref{lex.name})、
\grammarterm{操作符函数标识符}~(\ref{over.oper})、
\grammarterm{字面值操作符标识符}~(\ref{over.literal})、
\grammarterm{转换函数标识符}~(\ref{class.conv.fct})或
\grammarterm{模板标识符}~(\ref{temp.names})
的一次引用。

\pnum
表示实体的名称都由\term{声明}确立。
表示标号的名称要么由\tcode{goto}语句~(\ref{stmt.goto})确立，
要么由\grammarterm{标号语句}~(\ref{stmt.label})确立。

\pnum
\defn{变量}
由对象的声明确立，或由不作为非静态数据成员的引用的声明确立。
变量若存在名称，则该名称就表示该对象或引用。
...
先确定该名称是否表示这些实体中的某一个，
之后才能继续分析。
确定其含义的这一过程称为\indextext{lookup!name}%
\term{名称查找}~(\ref{basic.lookup})。

\pnum
两个名称\term{相同}，当

\begin{itemize}
\item 两者都是由相同字符序列组成的\grammarterm{标识符}{}，或
\item 两者都是由同一操作符形成的\grammarterm{操作符函数标识符}{}，或
\item 两者都是由同一类型形成的\grammarterm{转换函数标识符}{}，或
\item 两者都是指向同一个类、函数或变量(\ref{temp.type})的\grammarterm{模板标识符c}{}，或
\item 两者都是由相同的字面值后缀标识符形成的字面值操作符(\ref{over.literal})的名称。
\end{itemize}
