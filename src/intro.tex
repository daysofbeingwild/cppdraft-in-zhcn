...
\footnote{
在第\ref{\firstlibchapter}到第\ref{\lastlibchapter}章
以及第\ref{diff.library}章作出的限制下，
C标准库是\Cpp 标准库的子集。
}
...
\pnum
\ref{definitions}
额外定义了仅用于第\ref{library}章到第\ref{\lastlibchapter}章以及附录\ref{depr}的术语。
...
\definition{诊错信息}{defns.diagnostic}
...
指向派生自\tcode{B} (第\ref{class.derived}章)的\tcode{D}类对象，
...
\defncontext{模板} \grammarterm{template-parameter}

\indexdefn{signature}%
\definition{签名}{defns.signature}
\defncontext{函数} 名称、形参类型列表(\ref{dcl.fct})以及外围命名空间（如果有的话）\\
\enternote 签名是name mangling和链接的基础。 \exitnote

\indexdefn{signature}%
\definition{签名}{defns.signature.templ}
\defncontext{函数模板} 名称、形参类型列表(\ref{dcl.fct})、
外围命名空间（如果有的话）、返回类型以及模板形参列表
...
\defncontext{类成员函数} 名称、形参类型列表(\ref{dcl.fct})、
所在类、\cv 限定符（如果有的话）以及\grammarterm{ref-qualifier}（如果有的话）

\indexdefn{signature}%
\definition{签名}{defns.signature.member.templ}
\defncontext{类成员函数模板} 名称、形参类型列表(\ref{dcl.fct})、
所在类、\cv 限定符（如果有的话）、\grammarterm{ref-qualifier}（如果有的话）、
返回类型（如果有的话）以及模板形参列表
...
按照语法规则、可判断的语义规则以及一处定义规则(\ref{basic.def.odr})
构造的\Cpp 程序。%
\indextext{definitions|)}
