...
\Cpp 实现的首要需求就是实现语言本身，因此本标准对\Cpp 也作出了定义。
...
\definition{有条件支持的}{defns.cond.supp}
实现不一定要支持的程序构造\\
\enternote 所有实现都会注明它不支持的所有规定为有条件支持的程序构造。 \exitnote

\indexdefn{message!diagnostic}%
\definition{诊错信息}{defns.diagnostic}
属于实现输出信息中\impldef{diagnostic message}的那一类的信息
